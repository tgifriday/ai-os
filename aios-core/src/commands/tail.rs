pub fn run(args: &[&str], cwd: &std::path::Path) -> crate::CommandOutput {
    ...
    if files.is_empty() {
        return crate::CommandOutput::error("tail: file required\n".into(), 1);
    }
    ...
    for path in &files {
        let content = match std::fs::read_to_string(path) {
            Ok(c) => c,
            Err(e) => return crate::CommandOutput::error(format!("tail: {}: {}\n", path.display(), e), 1),
        };
        let all: Vec<&str> = content.lines().collect();
        let skip = all.len().saturating_sub(n);
        let taken: Vec<&str> = all.into_iter().skip(skip).collect();
        if files.len() > 1 {
